...
class PatientGeneratingNumber
{
    /**
     * find Number Available */
    public static function findNewNumber($type, $objCode, $idPatient)
    {
        ...
        $fourthCode = date('y');
        $countNumber = self::runningNumber($findMC->start_by,$findMC->reset_type, $findMC->type, $findMC->initial_by, $objCode, $idPatient);
    ...
    private static function runningNumber($startType, $resetType, $numberType, $initialBy = null, $objCode = null, $idPatient)
    {
        if($startType == 'general' && $resetType == 'monthly')
        {
            return self::generalMonthly($numberType, $idPatient) == null ? 1 : self::generalMonthly($numberType, $idPatient);
        }
        if($startType == 'general' && $resetType == 'annually')
        {
            return self::generalAnnually($numberType, $idPatient) == null ? 1 : self::generalAnnually($numberType, $idPatient);
        }
        if($startType == 'spesific' && $resetType == 'monthly')
        {
            return self::spesificMonthly($initialBy, $numberType, $objCode, $idPatient) == null ? 1 : self::spesificMonthly($initialBy, $numberType, $objCode, $idPatient);
        }
        if($startType == 'spesific' && $resetType == 'annually')
        {
            return self::spesificAnnually($initialBy, $numberType, $objCode, $idPatient) == null ? 1 : self::spesificAnnually($initialBy, $numberType, $objCode, $idPatient);
        }
    }

    private static function spesificMonthly($initialBy, $numberType, $objCode = null, $patientId)
    {
        $number = 0;
        $myAvailable = GenerateNumber::where(['create_by' => $patientId, 'available' => true])->where(DB::raw("MONTH(created_at)"), date('m'))->where('type', $numberType)->orderBy('order', 'desc')->first();
        if ($myAvailable) {
        ...
        if (!$findCount) {
            ...
            if ($findCount->available) {
                if (strtotime($findCount->expire_at) > strtotime(date('Y-m-d H:i:s'))) {
                    if ($findCount->create_by == $patientId) {
    ...
    private static function spesificAnnually($initialBy, $numberType, $objCode = null, $patientId)
    {
        $number = 0;
        if($initialBy == 'subject')
        {
            $myAvailable = GenerateNumber::where(['create_by' => $patientId, 'available' => true])->where(DB::raw("YEAR(created_at)"), date('Y'))->where('type', $numberType)->where('initial', request()->auth->code)->orderBy('order', 'desc')->first();
            if ($myAvailable) {
                return $myAvailable->order;
            }
            ...
            $myAvailable = GenerateNumber::where(['create_by' => $patientId, 'available' => true])->where(DB::raw("YEAR(created_at)"), date('Y'))->where('type', $numberType)->where('initial', $objCode)->orderBy('order', 'asc')->first();
            if ($myAvailable) {
                return $myAvailable->order;
            }
    ...
    private static function generalMonthly($numberType, $patientId)
    {
        $number = 0;
        $myAvailable = GenerateNumber::where(['create_by' => $patientId, 'available' => true])->where(DB::raw("MONTH(created_at)"), date('m'))->where('type', $numberType)->orderBy('order', 'desc')->first();
        if ($myAvailable) {
    ...
    private static function generalAnnually($numberType, $patientId)
    {
        $number = 0;
        $myAvailable = GenerateNumber::where(['create_by' => $patientId, 'available' => true])->where(DB::raw("YEAR(created_at)"), date('Y'))->where('type', $numberType)->orderBy('order', 'desc')->first();
        if($myAvailable)
        {
            return $myAvailable->order;
        }
